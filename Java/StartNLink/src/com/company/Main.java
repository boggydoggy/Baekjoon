...
팀에 더해지는 능력치이다. 팀의 능력치는 팀에 속한 모든 쌍의 능력치 Sij의 합이다. Sij는 Sji와 다를 수도 있으며, i번 사람과 j번 사람이 같은 팀에 속했을 때, 팀에 더해지는 능력치는 Sij와 Sji이다.
...
public class Main {
    ...
    public static void teamMake(int cnt, int idx) {
        if(cnt == n / 2) {
            dfs();
            return;
        }
    ...
    public static void dfs() {
        ...
        int ans = Math.abs(startTeam - linkTeam);
